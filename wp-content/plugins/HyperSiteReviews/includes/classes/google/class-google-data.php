...
class GoogleDataHandler
{
    ...
    public static function get_selected_reviews($location_id, $page, $per_page)
    {
        ...
        if (self::location_exists($location_id)) {
            ...
            // Update the SQL query to include the is_selected condition
            return $wpdb->get_results($wpdb->prepare(
                "SELECT reviewer_display_name, reviewer_profile_photo_url, comment, star_rating, create_time, is_selected FROM {$wpdb->prefix}reviews WHERE location_id = %s AND is_selected = TRUE LIMIT %d OFFSET %d",
                $location_id,
                $per_page,
                $offset
            ), ARRAY_A);
