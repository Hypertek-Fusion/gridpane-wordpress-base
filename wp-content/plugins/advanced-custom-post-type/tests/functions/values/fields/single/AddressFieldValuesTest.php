...
class AddressFieldValuesTest extends AbstractTestCase
{
    ...
    public function can_add_acpt_meta_field_value()
    {
	    ...
	    foreach ($this->dataProvider() as $key => $value){
		    ...
		    $this->assertEquals('Via Latina 94, 00179 Roma', $acpt_field['address']);
	    }
    }
	...
	public function can_edit_acpt_meta_field_value()
	{
		foreach ($this->dataProvider() as $key => $value){
			$edit_acpt_meta_field_value = save_acpt_meta_field_value([
				...
				'box_name' => 'box_name',
				'field_name' => 'address',
				'value' => "Via Macedonia 14, 00179 Roma",
			]);
			...
			$this->assertEquals('Via Macedonia 14, 00179 Roma', $acpt_field['address']);
		}
	}
    ...
    public function can_display_acpt_meta_field()
    {
	    foreach ($this->dataProvider() as $key => $value){
		    ...
		    $this->assertStringContainsString('<iframe class="maps" width="100%" height="500" src="https://maps.google.com/maps', $acpt_field);
	    }
    }
