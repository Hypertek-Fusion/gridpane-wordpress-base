...
class UserFieldValueTest extends AbstractTestCase
{
	/**
	 * @test
	 */
	public function can_add_acpt_meta_field_value()
	{
		...
		foreach ($this->dataProvider() as $key => $value){
			...
			$this->assertInstanceOf(\WP_User::class, $acpt_field);
			$this->assertEquals($acpt_field->ID, $this->oldest_user_id);
		}
	}
	...
	public function can_edit_acpt_meta_field_value()
	{
		foreach ($this->dataProvider() as $key => $value){
			...
			$this->assertInstanceOf(\WP_User::class, $acpt_field);
			$this->assertEquals($acpt_field->ID, $this->getUserId(1));
		}
	}
