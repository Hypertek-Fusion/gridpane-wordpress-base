					...
					$filter = array_filter($liveData, function ($element) use ($typeValue, $belongsTo, $elementId, $fieldIndex, $blockIndex, $blockName) {
						...
						if($belongsTo === MetaTypes::OPTION_PAGE){
							...
							if($typeValue->hasParent() and $fieldIndex !== null){
								...
								$elementId = $elementId."_".$parentMetaField->getDbName().'['.$typeValue->getName().']['.$fieldIndex.'][value]';
							...
							if($typeValue->hasParentBlock() and $blockIndex !== null and $blockName !== null){
								...
								$elementId = $elementId."_".$parentMetaField->getDbName().'[blocks]['.$blockIndex.']['.$blockName.']['.$typeValue->getName().']['.$fieldIndex.'][value]';
