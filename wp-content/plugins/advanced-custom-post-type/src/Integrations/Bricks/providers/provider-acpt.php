...
class Provider_ACPT extends Base
{
	...
	private function register_tag($field, $parent_field = [])
	{
		...
		if(in_array(self::CONTEXT_LOOP, $contextsByType)){
			...
			// Relational fields
            elseif(in_array($type, [ MetaFieldModel::POST_OBJECT_TYPE, MetaFieldModel::POST_OBJECT_MULTI_TYPE, MetaFieldModel::POST_TYPE ])){
                $this->loop_tags[ $name ] = $tag;
            }
		}
				...
				case MetaFieldModel::PHONE_TYPE:
					...
					if(is_string($raw_acpt_value)){
						if(!empty($args) and $args[0] === 'string'){
							...
							if($context === 'link'){
								$value = 'tel:'.Phone::url($phone_url); //$filters object_type = ''
							} else {
								$value = '<a href="tel:'.Phone::url($phone_url).'" target="_blank">'.$raw_acpt_value.'</a>';
							}
						}
					}
	...
	private static function get_fields_by_context()
	{
		return [
			...
			MetaFieldModel::REPEATER_TYPE          => [ self::CONTEXT_LOOP, self::CONTEXT_TEXT ],
			...
			MetaFieldModel::TERM_OBJECT_TYPE       => [ self::CONTEXT_TEXT, self::CONTEXT_LINK ],
			MetaFieldModel::TERM_OBJECT_MULTI_TYPE => [ self::CONTEXT_TEXT, self::CONTEXT_LINK ],
			MetaFieldModel::USER_TYPE              => [ self::CONTEXT_TEXT, self::CONTEXT_LINK ],
			MetaFieldModel::USER_MULTI_TYPE        => [ self::CONTEXT_TEXT, self::CONTEXT_LINK ],
		];
	}
...
//			}
    ...
    public function set_loop_object( $loop_object, $loop_key, $query )
    {
        ...
        if ( in_array( $field['type'], [ MetaFieldModel::POST_OBJECT_MULTI_TYPE, MetaFieldModel::POST_OBJECT_TYPE, MetaFieldModel::POST_TYPE ]  ) ) {
            global $post;
