...
class Modal_Widget extends \Bricks\Element {
	...
	public function set_controls() {
		...
		// info message for click anywhere to close.
		$this->controls['typeInfo'] = array(
			'group'    => 'settings',
			'content'  => esc_html__( 'Should the user be able to click anywhere outside the modal to close it?', 'frames' ),
			'type'     => 'info',
			'required' => false,
		);
	...
	public function render() {
		...
		if ( Flag::is_on( 'FRAMES_ENABLE_MODAL_EXIT_INTENT' ) ) {
			if ( isset( $settings['triggerType'] ) && true === $settings['triggerType'] && ! empty( $settings['userActionType'] ) ) {
		...
		if ( isset( $settings['triggerType'] ) && true === $settings['triggerType']  /*&& ! isset( $settings['triggerSelector'] )*/ ) {
		...
		// OUTPUT.
		$output = '<div ' . $this->render_attributes( '_root' ) . ' style="display: none;">';
